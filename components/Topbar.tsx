...
export function Topbar({ name, role, photoUrl, onMenu }: { name: string; role: string; photoUrl?: string | null; onMenu: () => void }) {
  ...
  return (
    <div className="h-[62px] bg-white border-b border-line flex items-center gap-3.5 px-5 sticky top-0 z-30">
      <button className="lg:hidden text-lg text-ink" onClick={onMenu}>☰</button>
      <div className="hidden md:flex items-center gap-2 bg-paper border border-line rounded-sm px-3 py-2 text-slate2-light text-sm flex-1 max-w-[400px]">
        <span>🔍</span>
        <input placeholder="Search students, IDs, payments…" className="bg-transparent outline-none text-sm w-full text-ink" />
      </div>
      ...
      <div className="flex items-center gap-2 pr-1">
        <div className="flex items-center gap-2 px-3 py-1 rounded-full border border-line">
          ...
          <div className="hidden sm:block leading-tight">
            ...
            <div className="text-[10.5px] text-slate2-light capitalize">{role.replace('_', ' ')}</div>
          </div>
        </div>
